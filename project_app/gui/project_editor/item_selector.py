...
def open_item_selector(parent, db_handler, callback):
    """
    Anyag-/terméklista választó ablak, teljesen reszponzív, storno/törlés támogatással, debuggal.
    """
    selected_items = []
    ...
    def update_selected_qty_entry(*args):
        ...
        if sel:
            # Mindig 1-re állítjuk vissza új sor kijelölésénél
            edit_qty_var.set(1)
        else:
            edit_qty_var.set(1)
    ...
    # Kijelölésnél resetel a storno mező
    ...
    # --- Lista szűrés, frissítés, hozzáadás + DEBUG ---
    def search_items(query):
        try:
            q = query.lower()
            items = db_handler.search_items(q)
            if not isinstance(items, list):
                print("[DEBUG] db_handler.search_items() nem listát adott vissza!")
                items = []
            print(f"[DEBUG] {len(items)} db találat adatbázisból a keresésre: '{q}'")
            return [
            ...
            print(f"[DEBUG] Hiba a keresés közben: {e}")
            return []
    ...
    def update_list():
        ...
        if not filtered:
            print(f"[DEBUG] Nincs találat a keresésre: '{search_var.get()}'")
    ...
    def add_to_selected():
        ...
        filtered = search_items(search_var.get())
        if not filtered:
            print("[DEBUG] A listában nincs kiválasztható elem.")
            return
        ...
        for i, (name, unit, price, q) in enumerate(selected_items):
            if name == item['nev'] and unit == item['egyseg']:
                ...
                update_selected_list()
                qty_var.set(1)  # Hozzáadás után vissza 1-re!
                print(f"[DEBUG] Létező tétel mennyisége nőtt: {name} {unit} +{qty}")
                return
        selected_items.append((item['nev'], item['egyseg'], item['egysegar'], qty))
        print(f"[DEBUG] Új tétel hozzáadva: {item['nev']} {item['egyseg']} x{qty}")
        update_selected_list()
        qty_var.set(1)  # Hozzáadás után vissza 1-re!
